...
async fn run_test_ceremony(store: Arc<Memory>) -> (Arc<SoftwareToken>, Arc<SoftwareToken>, x509_cert::Certificate, x509_cert::Certificate) {
    ...
    assert!(hierarchy.created, "première cérémonie : la hiérarchie doit être créée");
    (root_signer, issuing_signer, hierarchy.root, hierarchy.issuing)
}
...
async fn ceremony_is_idempotent() {
    ...
    assert!(!hierarchy2.created, "seconde cérémonie : la hiérarchie ne doit pas être recréée");
    assert_eq!(root1.to_der().unwrap(), hierarchy2.root.to_der().unwrap());
    assert_eq!(issuing1.to_der().unwrap(), hierarchy2.issuing.to_der().unwrap());

    let authorities: Vec<_> = [AUTHORITY_ROOT, AUTHORITY_ISSUING].into_iter().collect();
    for name in authorities {
        store.authority(name).await.expect("l'autorité doit être persistée");
    }
}
...
async fn ceremony_rejects_mismatched_signer_on_replay() {
    ...
    assert!(err.is_err(), "une clé de token différente de celle déjà scellée doit être rejetée");
}
...
async fn issue_produces_a_certificate_signed_by_the_issuing_key() {
    ...
    let public_key_der = end_entity.public_key_der().expect("clé publique de l'entité finale");
    let tsa_profile = profile::tsa_signer();

    let cert = issuer.issue(&public_key_der, "tsu.example.test", &tsa_profile, "txn-1").await.expect("l'émission doit réussir");
...
async fn revoke_is_idempotent_and_keeps_first_reason() {
    ...
    assert_eq!(stored.revocation_reason, 1, "la première raison de révocation doit être conservée");
}
...
async fn openssl_accepts_the_chain_and_honors_revocation() {
    ...
    let verify_after = std::process::Command::new("openssl")
        ...
        .output()
        .expect("openssl doit s'exécuter");
    assert!(!verify_after.status.success(), "openssl doit rejeter le certificat révoqué");
    let combined = format!("{}{}", String::from_utf8_lossy(&verify_after.stdout), String::from_utf8_lossy(&verify_after.stderr));
    assert!(combined.to_lowercase().contains("revoked"), "le rejet doit être motivé par la révocation, pas par une autre erreur : {combined}");
